...
const dataAction = ()=> async (dispatch:Dispatch<TBlockAction | TErrorAction| TFetchAction> )=>{
dispatch({type:FETCH})
try {
    const response = await axios.get(`${basicRequestUrl}?limit=${limit}&offset=${offset}`)
const data:TBlockData[] = response.data
console.log(data)
